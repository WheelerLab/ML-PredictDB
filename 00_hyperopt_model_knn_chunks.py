...
#Read in the chunk Genes
gene_chunks = open("/home/pokoro/data/paper_hyperopt/RF/chr22_"+chunk, "rb")
gene_chunks = pickle.load(gene_chunks)
...
#take only the genes in the chunks by using the intersect
genes = snps_intersect(genes, gene_chunks)


# prepare files where to write the results for each ML method

#rf
open("/home/pokoro/data/paper_hyperopt/KNN/"+pop+"_knn_hyperopt_chr"+chrom+"_chunk"+
     chunk+".txt", "w").write("gene_id"+"\t"+"gene_name"+"\t"+"chr"+"\t"+
                              "best_hyperparam"+"\t")

for i in range(0, max_evals, 1):
     open("/home/pokoro/data/paper_hyperopt/KNN/"+pop+"_knn_hyperopt_chr"+chrom+
...
for gene in genes:
    ...
    if (type(cis_gt) != int) & (cis_gt.shape[1] > 0):
         ...
         best = fmin(fn=objective, space=knn_space, algo=algo,
         ...
         open("/home/pokoro/data/paper_hyperopt/KNN/"+pop+"_knn_hyperopt_chr"+

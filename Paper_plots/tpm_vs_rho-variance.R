...
gex <- mets_obs[,c(2:77)]
gex <- data.frame(gene=obs_genes, varTPM=apply(gex, 1, var))# apply is used to apply variance on each row. 
...
tiff("Z:/data/ml_paper_reviewers_corrections/paper_figs/FigS8_rho_vs_raw-tpm-variance.tiff", width = 24, height = 20, 
     units = 'cm', res = 300, compression = 'lzw')
ggscatter(df, x="varTPM", y="spearman", size=0.8, add = "reg.line", add.params = list(color="red", size=1), conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson", cor.coef.size = 2.5) +
  xlab("Average Variance in Raw Expression Levels") + ylab("Model Performance") + xlim(0,400) + theme_bw(16) + facet_grid(pop~MLmodel)
dev.off()
...
# }
# 
# tiff("Z:/data/ml_paper_reviewers_corrections/paper_figs/FigS11_rho_vs_normalized-tpm-variance.tiff", width = 24, height = 20, 
#      units = 'cm', res = 300, compression = 'lzw')
# ggscatter(data, x="varTPM", y="spearman", size=0.8, add = "reg.line", add.params = list(color="red", size=1), conf.int = TRUE, 
